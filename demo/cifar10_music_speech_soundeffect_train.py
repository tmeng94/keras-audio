...
def load_audio_path_label_pairs(max_allowed_pairs=None):
    ...
    audio_paths = []
    with open('./data/lists/music_speech_soundeffect/dataset.txt', 'rt') as file:
        for line in file:
            audio_path = './very_large_data/' + line.strip()
            audio_paths.append(audio_path)
    pairs = []
    with open('./data/lists/music_speech_soundeffect/labels.txt', 'rt') as file:
        for line in file:
            label = int(line)
...
def main():
    ...
    batch_size = 8
    epochs = 100
    history = classifier.fit(audio_path_label_pairs, model_dir_path='./models_music_speech_soundeffect', batch_size=batch_size, epochs=epochs)
